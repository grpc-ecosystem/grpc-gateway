...
func findServicesMessages(s []*descriptor.Service, reg *descriptor.Registry, m messageMap, e enumMap) {
	for _, svc := range s {
		for _, meth := range svc.Methods {
			m[fullyQualifiedNameToSwaggerName(meth.RequestType.FQMN(), reg)] = meth.RequestType
			findNestedMessages(meth.RequestType, reg, m, e)
			m[fullyQualifiedNameToSwaggerName(meth.ResponseType.FQMN(), reg)] = meth.ResponseType
			findNestedMessages(meth.ResponseType, reg, m, e)
		}
	}
}

// findNestedMessages that can be generated by the services.
func findNestedMessages(message *descriptor.Message, reg *descriptor.Registry, m messageMap, e enumMap) {
	...
	for _, t := range message.Fields {
		...
		if fieldType != "" {
			if _, ok := m[fieldType]; !ok {
				...
				m[fieldType] = msg
				findNestedMessages(msg, reg, m, e)
			}
		}
	}
}
...
func fullyQualifiedNameToSwaggerName(fqmn string, reg *descriptor.Registry) string {
	return resolveFullyQualifiedNameToSwaggerName(fqmn, append(reg.GetAllFQMNs(), reg.GetAllFQENs()...))
}
...
func applyTemplate(p param) (string, error) {
	...
	m := messageMap{}
	e := enumMap{}
	findServicesMessages(p.Services, p.reg, m, e)
