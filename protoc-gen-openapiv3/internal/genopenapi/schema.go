...
func (b *schemaBuilder) fieldSchema(field *descriptor.Field) *SchemaOrRef {
	...
	if field.GetType() == descriptorpb.FieldDescriptorProto_TYPE_MESSAGE {
		...
		if err != nil {
			grpclog.Infof("protoc-gen-openapiv3: cannot resolve message %q for repeated field %q: %v; emitting open object array",
				field.GetTypeName(), field.GetName(), err)
			return &SchemaOrRef{
				Value: &Schema{
					Type:  SchemaType{"array"},
					Items: &SchemaOrRef{Value: &Schema{Type: SchemaType{"object"}}},
				},
			}
		}
...
func (b *schemaBuilder) scalarOrRef(field *descriptor.Field) *SchemaOrRef {
	...
	case descriptorpb.FieldDescriptorProto_TYPE_MESSAGE:
		...
		msg, err := b.reg.LookupMsg("", field.GetTypeName())
		if err != nil {
			grpclog.Infof("protoc-gen-openapiv3: cannot resolve message %q for field %q: %v; emitting open object",
				field.GetTypeName(), field.GetName(), err)
			return &SchemaOrRef{Value: &Schema{Type: SchemaType{"object"}}}
		}
		b.ensureMessageSchema(msg)
	...
	case descriptorpb.FieldDescriptorProto_TYPE_ENUM:
		...
		if err != nil {
			grpclog.Infof("protoc-gen-openapiv3: cannot resolve enum %q for field %q: %v; emitting plain string",
				field.GetTypeName(), field.GetName(), err)
			return &SchemaOrRef{Value: &Schema{Type: SchemaType{"string"}}}
		}
...
func (b *schemaBuilder) propertySchema(field *descriptor.Field) *SchemaOrRef {
	...
	if prop.Ref != "" {
		// 3.1.0 allows description as a sibling of $ref, but title must live
		// on a real schema body — so an annotation that sets a title, like
		// readOnly/writeOnly/deprecated, forces an allOf wrapper.
	...
	if prop.Value != nil {
		...
		applySchemaTitle(prop.Value, ann)
	}
	return prop
}
