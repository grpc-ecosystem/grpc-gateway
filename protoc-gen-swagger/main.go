...
var (
	...
	versionFlag                = flag.Bool("version", false, " the current verison")
	allowRepeatedFieldsInBody  = flag.Bool("allow_repeated_fieldprints_in_body", false, "allows to use repeated field in `body` and `response_body` field of `google.api.http` annotation option")
