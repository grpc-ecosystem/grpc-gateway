...
func buildParameters(b *schemaBuilder, m *descriptor.Method, binding *descriptor.Binding, pathParams []pathParam) []*ParameterRef {
	...
	for _, field := range m.RequestType.Fields {
		...
		params = append(params, b.queryParameters(field, "", cycle)...)
	}
	return params
}
...
func (b *schemaBuilder) queryParameters(field *descriptor.Field, prefix string, cycle *queryCycleChecker) []*ParameterRef {
	...
	if field.GetType() == descriptorpb.FieldDescriptorProto_TYPE_MESSAGE &&
		...
		switch {
		case err != nil:
			log.Printf("protoc-gen-openapiv3: cannot resolve message %q for query parameter %q: %v; emitting opaque parameter",
		...
		case msg.GetOptions().GetMapEntry():
			...
			return mapQueryParameter(b, field, name, msg)
		...
		default:
			if !cycle.enter(msg.FQMN()) {
				log.Printf("protoc-gen-openapiv3: query parameter %q exceeds recursion depth (%d); truncating",
					name, cycle.limit)
				return nil
			}
			...
			for _, nested := range msg.Fields {
				out = append(out, b.queryParameters(nested, name+".", cycle)...)
			}
			return out
		}
	}
...
func mapQueryParameter(b *schemaBuilder, field *descriptor.Field, name string, entry *descriptor.Message) []*ParameterRef {
	...
	if !ok {
		log.Printf("protoc-gen-openapiv3: map query parameter %q has unsupported key type %s; skipping",
