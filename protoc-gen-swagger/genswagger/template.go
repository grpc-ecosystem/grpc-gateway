...
// parse so this is effectively a no-op since the enums object is never
// created.
func findEnumerations(enums []*pbdescriptor.EnumDescriptorProto, d swaggerDefinitionsObject) {
	for _, enum := range enums {
		...
		// TODO: When grpc-gateway supports enums, extract the possible values for the enums.
		// At this time I can't determine the proper way to go from enum.GetName to "FQEN".
		d[enum.GetName()] = swaggerSchemaObject{
			Type: "string",
			Enum: enumNames,
		}
	}
...
// findServicesMessages discovers all the messages defined in the RPC methods of the service.
...
func applyTemplate(p param) (string, error) {
	...
	findEnumerations(p.GetEnumType(), s.Definitions)

	// Find all the service's messages that are defined (recursively) and then
