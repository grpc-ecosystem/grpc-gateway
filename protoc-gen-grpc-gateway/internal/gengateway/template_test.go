...
func TestApplyTemplateRequestWithoutClientStreaming(t *testing.T) {
	...
	for _, spec := range []struct {
	...
	}{
		{
			...
			sigWant:         `func request_ExampleService_Echo_0(ctx context.Context, unmarshaler runtime.Unmarshaler, client ExampleServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {`,
		},
		{
			serverStreaming: true,
			sigWant:         `func request_ExampleService_Echo_0(ctx context.Context, unmarshaler runtime.Unmarshaler, client ExampleServiceClient, req *http.Request, pathParams map[string]string) (ExampleService_EchoClient, runtime.ServerMetadata, error) {`,
		},
	} {
		...
		if want := `marshaler.NewDecoder(ctx, newReader()).Decode(&protoReq.GetNested().Bool)`; !strings.Contains(got, want) {
			t.Errorf("applyTemplate(%#v) = %s; want to contain %s", file, got, want)
		}
...
func TestApplyTemplateInProcess(t *testing.T) {
	...
	for _, spec := range []struct {
	...
	}{
		{
			...
			serverStreaming: false,
			sigWant: []string{
				`func local_request_ExampleService_Echo_0(ctx context.Context, unmarshaler runtime.Unmarshaler, server ExampleServiceServer, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {`,
