...
func Register{{$svc.GetName}}{{$.RegisterFuncSuffix}}Server(ctx context.Context, mux *runtime.ServeMux, server {{$svc.InstanceName}}Server) error {
	...
	{{- end }}
		...
		{{- if $b.PathTmpl }}
		rctx, err := runtime.AnnotateIncomingContext(ctx, mux, req, "/{{$svc.File.GetPackage}}.{{$svc.GetName}}/{{$m.GetName}}", runtime.WithPattern("{{$b.PathTmpl}}"))
		{{- else -}}
...
func Register{{$svc.GetName}}{{$.RegisterFuncSuffix}}Client(ctx context.Context, mux *runtime.ServeMux, client {{$svc.InstanceName}}Client) error {
	...
	{{- end }}
		...
		inboundMarshaler, outboundMarshaler := runtime.MarshalerForRequest(mux, req)
		{{- if $b.PathTmpl }}
		rctx, err := runtime.AnnotateContext(ctx, mux, req, "/{{$svc.File.GetPackage}}.{{$svc.GetName}}/{{$m.GetName}}", runtime.WithPattern("{{$b.PathTmpl}}"))
		{{- else -}}
